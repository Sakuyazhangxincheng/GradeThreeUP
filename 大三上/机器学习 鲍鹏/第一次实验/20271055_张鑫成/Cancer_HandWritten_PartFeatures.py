...
import pickle as pk
def save(DT):
    pk.dump(DT,open("Cancer2.dat","wb"))

def read():
    load = pk.load(open("Cancer2.dat","rb"))
    return load

def load_data():
    firstdata = np.genfromtxt("breast-cancer-wisconsin.data",delimiter=",")[0:, 1:]
    ...
    best = SelectKBest(f_regression, k=8)
